...
Options parse_args(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        if ((strcmp(argv[i], "--m") == 0 || strcmp(argv[i], "-m") == 0) && i + 1 < argc) {
        ...
        } else if ((strcmp(argv[i], "--tile-size") == 0 || strcmp(argv[i], "-ts") == 0) && i + 1 < argc) {
            ...
            std::cout << "Usage: ./dgemm [--m int] [--n int] [--k int] [--tile-size int] [--impl baseline|naive|tiled|cublas] [--no-verify]\n";
...
int main(int argc, char** argv) {
    ...
    check_cuda(cudaMemcpy(d_a, h_a.data(), bytes_a, cudaMemcpyHostToDevice), "cudaMemcpy h_a to d_a");
